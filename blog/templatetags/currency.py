    ...
    for item in items:
        try:
            ...
            if float(price_change) < 0:
                ...
                change_color = 'green'
            # change_color = item.find('span', class_='price-change').get('style')
        except:
            symbol = None
            price = None
...
def get_common_info(items):
    data = []
    table_title = items.find('div', class_='summary-table--title').find('h4').text
    table_title_base_curr = items.find('div', class_='summary-table--title').find('span').text
    curr_column = items.find('div', class_='summary-table--data').find_all('div', class_='summary-table--column')
    # data.append({
    #     'table_title' : table_title,
    #     'table_title_base_curr' : table_title_base_curr
    # })
...
def show_currency(block_class='currency-wrapper'):
    ...
    a = {'currency': currency, 'petrol_data': petrol_data, 'metal_data': metal_data, 'crypto_data': crypto_data, 'block_class': block_class}
    return a
